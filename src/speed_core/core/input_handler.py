...
if TYPE_CHECKING:
    ...
    from speed_core.core.position_manager import PositionManager
...
class InputHandler(QObject):
    ...
    def __init__(self, 
        ...
        self.logger = logging.getLogger("speed_core.Core.InputHandler")
    ...
    def _save_dragged_position(self) -> None:
        ...
        - Lock Position ON: Saves absolute X/Y. These become the authoritative coordinates
          restored on every startup, explorer restart, and display change.
        - Free Move ON:  Same — saves absolute X/Y coordinates.
        - Both OFF: Calculates and saves the offset relative to the tray/edge.
        """
        try:
            config = self.widget.config
            is_locked = config.get("lock_position", False)
            is_free_move = config.get("free_move", False)
            updates = {}

            if is_locked or is_free_move:
                # Save absolute coordinates so they survive reboots and explorer restarts
                ...
                from speed_core.utils.taskbar_utils import get_taskbar_info
                from speed_core import constants
