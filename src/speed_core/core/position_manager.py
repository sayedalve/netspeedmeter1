...
from speed_core.utils import taskbar_utils
from speed_core.utils.taskbar_utils import TaskbarInfo, get_taskbar_info, is_small_taskbar

if TYPE_CHECKING:
    from speed_core.views.widget import NetworkSpeedWidget

# Logger Setup
logger = logging.getLogger("speed_core.Core.PositionManager")
...
class PositionManager(QObject):
    ...
    def _apply_saved_position(self) -> bool:
        """
        Applies saved position if 'lock_position' OR 'free_move' is enabled.
        
        lock_position: User-chosen coordinates are authoritative. Automatic
        positioning logic (tray changes, taskbar icon shifts) never overrides them.
        free_move:     Legacy mode — saves on drag, restores on startup like
                       lock_position, but automatic positioning can still run
                       when the widget is not visible (visibility checks in
                       _execute_refresh call update_position only when free_move
                       is False, so lock_position is the stronger guarantee).
        """
        is_free_move = self._state.config.get('free_move', False)
        is_locked = self._state.config.get('lock_position', False)

        if not is_free_move and not is_locked:
            return False
    ...
    def _check_for_tray_changes(self) -> None:
        ...
        # Skip automatic repositioning when the user has locked the position OR is in free-move mode.
        is_locked = self._state.config.get("lock_position", False)
        is_free_move = self._state.config.get("free_move", False)
        if is_locked or is_free_move or not self._state.widget.isVisible():
            return
    ...
    def constrain_drag(self, pos: QPoint) -> QPoint:
        ...
        In lock_position or free_move mode: constrain to screen bounds only.
        In auto mode: snap/constrain to taskbar safe zone.
        """
        ...
        if is_locked or is_free_move:
            # Allow dragging freely anywhere on any connected monitor
