"""
Helper utilities for speed_core.
...
def setup_logging() -> logging.Logger:
    ...
    with _logging_lock:
        if not logger.handlers: # Check if handlers are already configured
            ...
            is_production = os.environ.get(getattr(constants.app, 'ENV_VAR_PROD_MODE', 'SPEED_CORE_PROD'), "").lower() == "true"
    ...
    if current_speed < network_consts.MINIMUM_DISPLAY_SPEED:
        ...
        unit = labels[2]
            ...
            if speed_value >= giga_div:
                ...
                # Custom Modification: Force KB/s as the absolute minimum, eliminating B/s
