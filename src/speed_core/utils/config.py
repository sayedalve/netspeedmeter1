"""
Configuration management for speed_core.
...
class ConfigManager:
    """
    Manages loading, saving, and validation of speed_core's configuration.
    """
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None) -> None:
        ...
        self.logger = logging.getLogger("speed_core.Config")
    ...
    def _migrate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._version_less_than(loaded_version, "1.2"):
            # v1.2: Added lock_position. Default existing users to False so behaviour
            # is unchanged — they can opt in via Settings.
            if "lock_position" not in migrated:
                migrated["lock_position"] = False
                changes_made = True
                self.logger.info("Migration v1.2: added 'lock_position' = False")
    ...
    def setup_logging(cls, log_level: str = 'INFO') -> None:
        ...
        try:
            ...
            # Use the root logger to catch all logs in the process (including speed_core and speed_core)
