...
class TestNetworkMonitorThread:
    ...
    def test_successful_poll_resets_error_count(self, monitor_thread):
        ...
        with patch('speed_core.core.monitor_thread.psutil.net_io_counters') as mock_psutil:
    ...
    def test_transient_error_increments_count(self, monitor_thread):
        ...
        with patch('speed_core.core.monitor_thread.psutil.net_io_counters', side_effect=OSError("Test Error")):
            monitor_thread.start()
            time.sleep(0.05)
            monitor_thread.stop()
    ...
    def test_circuit_breaker_trips(self, q_app):
        ...
        with patch('speed_core.core.monitor_thread.constants.timers.MINIMUM_INTERVAL_MS', 10):
            monitor_thread = StatsMonitorThread(interval=0.01)
            
            with patch('speed_core.core.monitor_thread.psutil.net_io_counters', side_effect=OSError("Persistent Error")):
