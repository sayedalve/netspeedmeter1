...
def mock_hooks():
    ...
    with patch('speed_core.core.system_events.WinEventHook') as MockHook:
...
def test_start_creates_hooks(system_handler, mock_hooks):
    ...
    with patch('speed_core.core.system_events.get_taskbar_info') as mock_get_tb:
...
def test_immediate_hide_signal_emitted(system_handler):
    ...
    with patch('speed_core.core.system_events.get_taskbar_info') as mock_get_tb, \
         patch('speed_core.core.system_events.is_taskbar_obstructed', return_value=True), \
