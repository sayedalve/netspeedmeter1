...
from speed_core.views.app_activity.worker import AppActivityWorker
...
def test_sample_groups_by_pid_and_calculates_deltas():
    ...
    with (
        patch("speed_core.views.app_activity.worker.psutil.net_connections", side_effect=fake_net_connections),
        patch("speed_core.views.app_activity.worker.psutil.Process", side_effect=lambda pid: FakeProcess(pid)),
        patch("speed_core.views.app_activity.worker.time.monotonic", side_effect=[100.0, 101.0]),
    ):
...
def test_sample_handles_empty_connections():
    ...
    with (
        patch("speed_core.views.app_activity.worker.psutil.net_connections", return_value=[]),
        patch("speed_core.views.app_activity.worker.time.monotonic", return_value=100.0),
    ):
        worker.sample()
...
def test_sample_falls_back_when_global_connections_are_denied():
    ...
    with (
        patch("speed_core.views.app_activity.worker.psutil.net_connections", side_effect=psutil.AccessDenied()),
        patch(
            "speed_core.views.app_activity.worker.psutil.process_iter",
