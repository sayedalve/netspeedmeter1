...
class NetworkSpeedWidget(QWidget):
    ...
    def _execute_refresh(self, hwnd: int = 0) -> None:
        ...
        try:
            ...
            if self.isVisible():
                # Skip auto-positioning in free_move mode (user has full control, position
                # is only ever set by dragging). For lock_position and the default auto mode,
                # always call update_position so PositionManager can either restore the saved
                # coordinates (lock) or recalculate from tray geometry (auto).
    ...
    def _setup_connections(self) -> None:
        ...
        try:
            ...
            # 2. Monitor / display-change signals.
            # When screens are added, removed, or the primary changes, re-apply the
            # saved position so the widget doesn't land off-screen or in the wrong spot.
            _app = QApplication.instance()
            if _app:
                _app.screenAdded.connect(
                    lambda _: QTimer.singleShot(500, self._on_display_changed))
                _app.screenRemoved.connect(
                    lambda _: QTimer.singleShot(500, self._on_display_changed))
                _app.primaryScreenChanged.connect(
                    lambda _: QTimer.singleShot(500, self._on_display_changed))
    ...
    def _on_display_changed(self) -> None:
        """
        Called when monitors are added, removed, or the primary screen changes.

        Re-applies the saved/locked position so the widget doesn't end up off-screen.
        In lock_position mode the saved absolute coordinates are authoritative; in
        auto mode the position is recalculated from the (possibly new) tray geometry.
        """
        self.logger.debug("Display configuration changed — re-applying widget position.")
        try:
            self.position_manager.update_position()
            self._ensure_win32_topmost()
        except Exception as e:
            self.logger.error("Error handling display change: %s", e, exc_info=True)
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            if self.config.get("free_move", False) or self.config.get("lock_position", False):
                # Persist the widget's current on-screen position so it is restored
                # exactly on the next startup (covers both Free Move and Lock Position).
                pos = self.pos()
