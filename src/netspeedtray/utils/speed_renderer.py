...
class SpeedRenderer:
    ...
    def preferred_size(self) -> tuple[int, int]:
        ...
        val_w   = max(
            self._metrics.horizontalAdvance(sample_up),
            self._metrics.horizontalAdvance(sample_down),
        )
        unit_w  = self._metrics.horizontalAdvance("Mbps")
        total_w = arrow_w + 3 + val_w + 3 + unit_w + 8
        row_h   = self._metrics.height() + 1
        total_h = row_h * 2 + 2
    ...
    def _build_fonts(self) -> None:
        cfg = self.cfg
        self._font = QFont(cfg.font_family, cfg.font_size, cfg.font_weight)
        self._metrics = QFontMetrics(self._font)
        arrow_weight = min(cfg.font_weight + 200, 900)
        self._arrow_font = QFont(cfg.font_family, cfg.font_size, arrow_weight)
    ...
    def _draw_row_at(
        ...
        unit_w  = self._metrics.horizontalAdvance(unit_str)
